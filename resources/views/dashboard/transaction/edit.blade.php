            ...
            <form id="updateFormTransaction" data-id="{{ $transaction->id }}">
                    ...
                    <div class="row">
                                ...
                                <select name="user_id" class="form-control" id="user_id">
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ $user->id == $selectedUserIds ? 'selected' : '' }}>{{ $user->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-12 col-md-6">
                            ...
                            <select name="status" class="form-control" id="status">
                                ...
                                <option value="rejected" {{ $transaction->status == 'rejected' ? 'selected' : '' }}>Tolak
                                </option>
                                <option value="approved" {{ $transaction->status == 'approved' ? 'selected' : '' }}>Selesai
                                </option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div id="dynamic-input-sampah">
                        @foreach ($transactionDetails as $key => $detail)
                            <div class="row align-items-center input-group-sampah">
                                        ...
                                        <select name="sampah_id[]" class="form-control sampah-select">
                                            ...
                                            @foreach ($sampahs as $sampah)
                                                <option value="{{ $sampah->id }}" data-harga="{{ $sampah->harga }}" data-points="{{$sampah->points}}"
                                                    ...
                                                    {{ $sampah->nama }} - Rp
                                                    {{ number_format($sampah->harga, 0, ',', '.') }}/kg
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="col-1">
                                    @if (!$loop->first)
                                        <button type="button" class="btn btn-danger remove-row-sampah"><i
                    ...
                    <div class="row">
                        <div class="col-12 col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control shadow-sm" name="total_amount" id="total_amount"
                                    value="{{ number_format($transaction->total_amount, 0, ',', '.') }}" readonly>
                            </div>
                        </div>
                        <div class="col-12 col-md-6">
                            <div class="form-group">
                                <label for="total_points">Total Points</label>
                                <input type="text" class="form-control shadow-sm" name="total_points" id="total_points"
                                    value="{{ number_format($transaction->total_points, 0, ',', '.') }}" readonly>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-footer bg-light">
                    <div class="d-flex justify-content-start">
                        ...
                        <button type="button" onclick="window.location.href='{{ route('transaction.index') }}'"
