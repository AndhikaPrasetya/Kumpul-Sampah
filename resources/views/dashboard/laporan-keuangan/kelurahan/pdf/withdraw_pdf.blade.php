...
<head>
    <meta charset="UTF-8">
    <title>Laporan Penarikan Saldo - Kelurahan {{ strtoupper($user->user->name) }}</title>
    <style>
        body { 
...
<body>
        ...
        <tbody>
            @foreach($data as $index => $withdraw)
                <tr>
                    ...
                    <td>{{ $withdraw->user->bsuDetail->name ?? '-' }}</td>
            ...
            <tr class="total-row">
                ...
                <td colspan="2">Rp {{ number_format($data->sum('amount'), 0, ',', '.') }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="footer">
        ...
        KELURAHAN {{ strtoupper($user->user->name) }} - Mewujudkan Lingkungan Bersih dan Sejahtera
    </div>
</body>
</html>
